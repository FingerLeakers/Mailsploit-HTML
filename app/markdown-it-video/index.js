...
function videoUrl(service, videoID, options) {
  switch (service) {
    case 'youtube':
      let urlOptions = '';
      Object.keys(options.youtube).map(function(key, index) {
          if (key === 'width' || key === 'height') {
            return '';
          }
          urlOptions = `${urlOptions}&${key}=${options.youtube[key]}`;
      });
      return 'https://www.youtube.com/embed/' + videoID + '?' + urlOptions;
    case 'vimeo':
...
const defaults = {
  url: videoUrl,
  youtube: { width: 640, height: 390, theme: 'white', rel: '0', controls: '0', showinfo: '0' },
